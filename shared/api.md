> ⚠️ **24h 조건은 제품 스펙과 불일치 상태다 (2026-08-18 확인).**
> `domain.md`·`product-spec.md`는 2026-07-05에 **"시간 제한 없음 · 응답 3건 확보 즉시 결과 전환"** 으로 확정됐으나,
> 이 문서와 프론트 코드(`WAITING_RESULT_OPEN_TIME`, `resultAvailableAt`)에는 24시간 게이트가 남아 있다.
> **이 문서는 백엔드 계약이므로 임의로 고치지 않는다** — 어느 쪽에 맞출지 백엔드와 합의한 뒤 갱신할 것.
> 추적: `shared/launch-checklist.md` 미정 항목.
