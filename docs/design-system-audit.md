...
⚠️ **3·5번은 컴포넌트 파일을 건드린다.** 2026-08-19 기준 테스트는 **195개**(17개 파일)로 늘었고 `[token]` 상태머신·fetch 래퍼·API 레이어(queries/mutations)·result-view·share-cards 는 characterization 테스트가 덮고 있다. 다만 **시각적 회귀를 잡는 테스트는 아직 없다**(Playwright 미도입) — 픽셀이 바뀌는 변경은 여전히 눈으로 확인해야 한다.
